...
def estructurar_salida_excel_estudiantes(df, origen):
    ...
    if origen == 'both':
        ...
        out['Estado'] = "Alumno a eliminar" if origen == 'A' else "Alumno sin Employee ID"
...
def procesar_estudiantes(nombre_archivo_a, nombre_archivo_b, directorio_base="", directorio_salida="resultado estudiantes", dicc_establecimientos=None, procesar_egresados=True):
    ...
    # 1. NORMALIZAR Y EXTRAER INFO EN ARCHIVO A (Workspace)
    ...
    # Se genera ID_A usando el Employee ID (RUT)
    df_a['ID_A'] = df_a['Employee ID'].apply(normalizar_rut)

    # SEPARAR REGISTROS DE WORKSPACE QUE NO TIENEN EMPLOYEE ID
    mask_sin_id = df_a['ID_A'] == ""
    df_a_sin_id = df_a[mask_sin_id].copy()
    df_a_con_id = df_a[~mask_sin_id].copy()

    df_a_con_id['ID'] = df_a_con_id['ID_A']

    # 2. NORMALIZAR Y OBTENER ID (RUT) EN ARCHIVO B (SIGE)
    ...
    # 3. CRUCE DE DATOS POR RUT (ID) SOLO PARA CUENTAS CON EMPLOYEE ID
    df_merge = pd.merge(df_a_con_id, df_b_validos, on='ID', how='outer', indicator=True)
    ...
    if not df_a_sin_id.empty:
        ...
        excel_sin_employee_id.to_excel(os.path.join(directorio_salida, 'solo_a_sin_employee_id.xlsx'), index=False)
        csv_sin_employee_id.to_csv(os.path.join(directorio_salida, 'solo_a_sin_employee_id.csv'), index=False, encoding='utf-8')
    ...
    print(f"➖ Alumnos a archivar/eliminar con Employee ID (Solo A/Workspace): {len(df_solo_a)}")
    if not df_a_sin_id.empty:
        print(f"⚠️ Alumnos Workspace sin Employee ID (exportados aparte): {len(df_a_sin_id)}")
...
if __name__ == "__main__":
    ...
    procesar_estudiantes(
        nombre_archivo_a="archivos alumno/uso_workspace2.xlsx", 
